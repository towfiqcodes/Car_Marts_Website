<html>
	...
	<div>
		<h1 style="color: white;text-align: center;"> Nissan GTR R35 </h1>
		
		<center>
		<img src= "image/nissan1.jpg" width="1320" height="300"  / ></center>
		...
		<div style="color: white; font-size:40px; text-align:center;"> Nissan Brake Parts</div><br><br>
	 	...
	 	<?php
			...
			$sql = "select parts.*,images.image_name from parts inner join categories on categories.id = parts.categories_id inner join cars on cars.id = parts.cars_id inner join images on parts.id = images.parts_id where parts.cars_id =10 and parts.categories_id=2";
			...
			if (!empty($result)) {
			    ...
			    while($row = $result->fetch_assoc()) {
			        ...
			        echo '<button><a href="registration.php">BUY</a></button>';
			        echo '</div>';
			    }
			} else {
			    echo "0 results";
